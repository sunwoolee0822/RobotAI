...
class EncoderLayer_GAT(nn.Module):
    ...
    def forward(self, x, mask, time):
        "Follow Figure 1 (left) for connections."
        # print('x',x.shape,'attn',self.self_attn(x,x,x,mask).shape)
        graphs = build_graph(time,x.device)
...
class EncoderDecoder(nn.Module):
    ...
    def __init__(self, encoder,src_pe
                ,generator,num_agents=23,num_neighbors=23,agent_encoding_dim=32,d_model=256):
    ...
    def forward(self, data, time, mask):
        ...
        memory_ = memory_.sum(2)
        feature_regular = feature_mask.sum(2)+1e-9
        agentwise_memory = memory_/feature_regular#(b,a,f)
...
def make_model(src_vocab, tgt_vocab, N=8, d_model=256, d_ff=1024, h=8, dropout=0.1,num_agents=23,num_neighbors=23,agent_encoding_dim=32,static_dim=6):
    ...
    model = EncoderDecoder(
        Encoder(EncoderLayer(d_model+agent_encoding_dim, c(attn), c(ff), dropout),EncoderLayer_GAT(d_model+agent_encoding_dim, c(attn_a), c(ff), dropout), N),
        c(position),
        Generator(d_model+agent_encoding_dim,d_model, tgt_vocab),
